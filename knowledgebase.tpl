        ...
        {foreach $kbcats as $category}
            <div class="col-xl-6">
                <div class="card card-body mb-2">
                    <a href="{routePath('knowledgebase-category-view', {$category.id}, {$category.urlfriendlyname})}" data-id="{$category.id}">
                        <span class="h6 m-0">
                        ...
                        <p class="m-0 text-muted"><small>{$category.description}</small></p>
                            {if $category.editLink}
                                <button class="btn btn-sm btn-warning" id="btnEditCategory-{$category.id}" data-url="{$category.editLink}" type="button">
                                    {lang key="edit"}
                                </button>
                            {/if}
                </div><br>
