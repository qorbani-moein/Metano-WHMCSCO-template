...
<div class="table-container table-responsive clearfix">
        <h1 class="text-primary">{$displayTitle}</h1>
    <table id="tableEmailsList" class="table table-list w-hidden">
        <thead>
            <tr>
                <th style="width: 50%;">{lang key='clientareaemailssubject'}</th>
        ...
        <tbody>
            {foreach $emails as $email}
                <tr onclick="popupWindow('viewemail.php?id={$email.id}', 'emailWin', '800', '800')">
                    <td style="width: 50%;">{$email.subject}{if $email.attachmentCount > 0} <i class="fal fa-paperclip"></i>{/if}</td>
